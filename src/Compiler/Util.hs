...
declMalloc =
  GlobalDefinition
    functionDefaults
    {name = Name "malloc", parameters = ([Parameter int (Name "len") []], False), returnType = int}
...
encChar c | '0' <= c && c <= '9' = fromIntegral $ ord c - ord '0'
encChar c | 'A' <= c && c <= 'Z' = fromIntegral $ ord c - ord 'A' + 10
encChar c | 'a' <= c && c <= 'z' = fromIntegral $ ord c - ord 'a' + 36
encChar '_' = 62
encChar '#' = 63
